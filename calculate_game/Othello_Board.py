...
class Othello():
    def __init__(self, master, n, calcBoard):
        '''コンストラクタ'''
        self.master = master  # 親ウィジェット
        self.calcBoard = calcBoard # 計算ゾーンのインスタンス
    ...
    def createWidgets(self):
        ...
        self.canvas = tkinter.Canvas(
            self.master,
            bg=BOARD_COLOR,
            width=CANVAS_SIZE_WIDTH+1,  # +1は枠線描画のため
            height=CANVAS_SIZE_HEIGHT+1,  # +1は枠線描画のため
    ...
    def initOthello(self):
        ...
        # １マスのサイズ（px）を計算
        self.square_size = CANVAS_SIZE_HEIGHT // NUM_SQUARE
    ...
    def click(self, event):
        ...
        if self.checkPlacable(x, y):
            self.calcBoard.deleteText("number")
            self.calcBoard.displayNumber(self.n[x][y], x, y)
            self.calcBoard.getInstance(self)
    ...
    def showResult(self):
        ...
        tkinter.messagebox.showinfo(
            '結果', 'あなた' + str(num_your) + '：COM' + str(num_com))
